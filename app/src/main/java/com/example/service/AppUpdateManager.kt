...
class AppUpdateManager(private val context: Context) {
    ...
    private val remoteConfig: FirebaseRemoteConfig? = try {
        ...
        android.util.Log.e("AppUpdateManager", "Firebase Remote Config is not available", e)
        null
    }
    ...
    init {
        ...
        try {
            ...
            android.util.Log.e("AppUpdateManager", "Failed to configure Remote Config", e)
        }
    }
        ...
        try {
            if (forceFetch) {
                ...
                val tempSettings = FirebaseRemoteConfigSettings.Builder()
                    ...
                    .build()
                config.setConfigSettingsAsync(tempSettings).await()
            }

            config.fetchAndActivate().await()

            val latestVersionCode = config.getLong("latest_version_code").toInt()
            val downloadUrl = config.getString("apk_download_url")
